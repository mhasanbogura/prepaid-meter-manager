...
function renderNescoHistory(m) {
  ...
  return `<section class="card">
    ...
    ${rows.length
      ? `<div class="tbl-scroll"><table class="list"><thead><tr><th>${esc(t('detail.date'))}</th><th>${esc(t('detail.token'))}</th><th style="text-align:right">${esc(t('detail.total'))}</th><th style="text-align:right">Energy (kWh)</th><th style="text-align:right">${esc(t('detail.vat'))}</th><th>${esc(t('detail.method'))}</th></tr></thead><tbody>
...
function renderHistory(hist) {
  ...
  return `<section class="card">
    <h3>${esc(t('detail.recharge_history'))}</h3>
    ${rows.length
      ? `<div class="tbl-scroll"><table class="list"><thead><tr><th>${esc(t('detail.date'))}</th><th>${esc(t('detail.token'))}</th><th style="text-align:right">${esc(t('detail.total'))}</th><th style="text-align:right">Energy Amount</th><th style="text-align:right">${esc(t('detail.vat'))}</th><th style="text-align:right">${esc(t('detail.rebate'))}</th></tr></thead><tbody>
  ...
  if (!state.meters.length) {
    c.innerHTML = demoBanner + `
      ...
      <div style="text-align:center;margin-top:80px;padding:16px 0">
        <span style="font-size:11px;color:var(--text-2);font-family:serif;letter-spacing:0.5px">Version 1.0.70 (build 213)</span>
      </div>`;
    return;
  }
