...
window._settingsContact = async function(el) {
  ...
  const icons = {
    ...
    messenger: '<svg width="18" height="18" viewBox="0 0 24 24" fill="#00B2FF"><path d="M12 2C6.477 2 2 6.145 2 11.243c0 2.907 1.438 5.502 3.681 7.18V22l3.436-1.885c.915.252 1.887.385 2.883.385 5.523 0 10-4.145 10-9.257C22 6.145 17.523 2 12 2zm1.062 12.517l-2.55-2.735-4.979 2.735 5.494-5.832 2.614 2.735 4.916-2.735-5.495 5.832z"/></svg>',
  ...
  let html = `<div style="text-align:center;margin-bottom:12px"><div style="width:80px;height:80px;border-radius:20px;background:var(--primary);color:#fff;display:flex;align-items:center;justify-content:center;font-size:32px;font-weight:700;margin:0 auto 8px;box-shadow:var(--shadow-lg)">${esc(developerName.charAt(0).toUpperCase())}</div><div style="font-weight:700;font-size:16px">${esc(developerName)}</div></div><div style="display:flex;flex-direction:column;gap:8px">`;
