...
const I18N = {
  en: {
    ...
    'nesco.no_data': 'NESCO returned no data for that consumer number.',
  ...
  bn: {
    ...
    'nesco.no_data': 'নেসকো সেই কনজিউমার নম্বরের কোনো তথ্য দেয়নি।',
...
function renderNescoHistory(m) {
  ...
  return `<section class="card">
    ...
    ${rows.length
      ? `<div class="tbl-scroll"><table class="list"><thead><tr><th>${esc(t('detail.date'))}</th><th>${esc(t('detail.token'))}</th><th style="text-align:right">${esc(t('detail.total'))}</th><th style="text-align:right">${esc(t('detail.energy'))}</th><th style="text-align:right">${esc(t('detail.vat'))}</th><th>${esc(t('detail.method'))}</th></tr></thead><tbody>
        ${rows.map(r => `<tr><td>${esc(r.rechargeDate || '')}</td><td style="color:var(--text-2);white-space:normal;word-break:break-all;max-width:140px">${(r.tokenNo || r.orderId || '').replace(/,?\s*&lt;br&gt;\s*/g, ', ').replace(/,\s*$/, '')}</td><td style="text-align:right;font-weight:600">${fmtBdt(r.rechargeAmount)}</td><td style="text-align:right">${fmtUnits(r.energyUnit)}</td><td style="text-align:right">${fmtBdt(r.vat)}</td><td>${esc(r.method || '')}</td></tr>`).join('')}
      </tbody></table></div>`
...
function renderHistory(hist) {
  ...
  return `<section class="card">
    <h3>${esc(t('detail.recharge_history'))}</h3>
    ${rows.length
      ? `<div class="tbl-scroll"><table class="list"><thead><tr><th>${esc(t('detail.date'))}</th><th>${esc(t('detail.token'))}</th><th style="text-align:right">${esc(t('detail.total'))}</th><th style="text-align:right">${esc(t('detail.energy'))}</th><th style="text-align:right">${esc(t('detail.vat'))}</th><th style="text-align:right">${esc(t('detail.rebate'))}</th></tr></thead><tbody>
        ${rows.map(r => `<tr><td>${esc(fmtDate(r.rechargeDate))}</td><td style="color:var(--text-2);white-space:normal;word-break:break-all;max-width:140px">${esc(r.orderID)}</td><td style="text-align:right;font-weight:600">${fmtBdt(r.totalAmount)}</td><td style="text-align:right">${fmtBdt(r.energyAmount)}</td><td style="text-align:right">${fmtBdt(r.VAT)}</td><td style="text-align:right">${fmtBdt(r.rebate)}</td></tr>`).join('')}
      </tbody></table></div>`
      : `<p class="muted">${esc(t('detail.recharge_empty'))}</p>`}
  </section>`;
}
...
function initUi() {
  ...
  document.addEventListener('click', e => {
    ...
    if (currentView === 'meter' && !e.target.closest('.meter-card') && !e.target.closest('.dialog') && !e.target.closest('.dialog-wrap') && !e.target.closest('.btn') && !e.target.closest('button') && !e.target.closest('a') && !e.target.closest('input') && !e.target.closest('select') && !e.target.closest('.card') && !e.target.closest('.tbl-scroll') && !e.target.closest('.chart-wrap')) {
